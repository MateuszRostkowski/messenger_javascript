class Chat {
  ...
  makeMessage(message) {
    ...
    messageContainer.className = `message-container ${
      message.messageTitle === this.user.email ? "user-message" : ""
    }`.trim();
    ...
    image.setAttribute(
      "src",
      message.image || `https://api.adorable.io/avatars/100/${message.email}`
    );
  ...
  sendMessage() {
    ...
    firebase
      ...
      .push({
        ...
        image: this.user.photoURL,
        today:
          this.addZero(thisDate.getHours()) +
          ":" +
          this.addZero(thisDate.getMinutes()),
        tooltip:
          thisDay +
          " " +
          new Intl.DateTimeFormat("en-US", options).format(thisDate),
  ...
  makeLoginBox() {
    ...
    header.innerHTML =
      'Messenger by <a href="https://github.com/MateuszRostkowski" target="_blank">MateuszRostkowski</a>';

    // add event listeners
    button.addEventListener("click", () => this.onLoginByGoogleClickHandler());
    // put it all together
